...
// Helper: Convert image file to Base64 "text" format
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        ...
        const base64DataUrl = await imageToBase64(compressedFile);
        setProductImage(base64DataUrl);
      } catch (err) {
    ...
    <div className="bg-brand-secondary p-6 rounded-lg shadow-lg relative">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <button
            type="submit"
            disabled={!productName || !customerName || !address || isLoading || isProcessingImage}
          ...
          <label
            ...
            className={`flex items-center bg-brand-accent text-brand-text-secondary font-bold py-3 px-6 rounded-lg transition duration-300 ${
              isProcessingImage ? 'cursor-wait bg-opacity-50' : 'hover:bg-opacity-80 cursor-pointer'
            }`}
          >
            <ImageIcon className="w-5 h-5 mr-2" />
            <span>{isProcessingImage ? 'Processing...' : productImage ? 'Change Image' : 'Add Image'}</span>
          </label>
